...
`;var x="tw-element-active";var L=()=>{[...document.querySelectorAll(`.${x}`)].forEach(n=>n.classList.remove(x))},v=n=>{L(),n.classList.add(x)};function B(){y();let n=setInterval(()=>{if(tailwindScreens?.length){let b=function(e){e.preventDefault(),t.className=p.value,d=h(t)};clearInterval(n);let r=document.createElement("div");r.innerHTML=`
    ...
    `,r.classList.add("fixed");let a=["right-4","bottom-4"];a.forEach(e=>r.classList.add(e)),document.body.appendChild(r);let l=document.getElementById("twaBreakpoint"),c=[...document.querySelectorAll('input[type="checkbox"]')],C=document.getElementById("twaClassesAdd"),p=document.getElementById("twaClassesEditor"),$=[...document.querySelectorAll("[data-position]")],u=[...document.querySelectorAll("[data-relative]")],E=document.getElementById("twaError"),t,d;document.addEventListener("click",e=>{let i=e.target;twaPopup.contains(i)||(twaPopup.open=!1),e.metaKey&&(e.preventDefault(),twaPopup.open=!0,t=i,l.innerText=k(),c.forEach(s=>s.checked=!0),p.value=t.className,d=h(t))}),document.addEventListener("keydown",function(e){if(twaPopup.open&&e.metaKey){let i=e.key;if(i==="Enter")e.preventDefault(),b(e);else if(i>="1"&&i<="9"){if(Array.from(c)?.length){e.preventDefault();let s=c[parseInt(i)-1];if(s){let o=new Event("input");s.dispatchEvent(o)}else[...document.querySelectorAll('input[type="checkbox"]:focus')].forEach(o=>o.blur())}}else if(i==="ArrowLeft"){e.preventDefault();let s=u.find(o=>o.getAttribute("data-relative")==="prev");if(s){let o=new Event("click");s.dispatchEvent(o)}}else if(i==="ArrowUp"){e.preventDefault();let s=u.find(o=>o.getAttribute("data-relative")==="parent");if(s){let o=new Event("click");s.dispatchEvent(o)}}else if(i==="ArrowRight"){e.preventDefault();let s=u.find(o=>o.getAttribute("data-relative")==="next");if(s){let o=new Event("click");s.dispatchEvent(o)}}else if(i==="ArrowDown"){e.preventDefault();let s=u.find(o=>o.getAttribute("data-relative")==="child");if(s){let o=new Event("click");s.dispatchEvent(o)}}}}),c.forEach(e=>{e.addEventListener("input",()=>{(typeof d=="object"&&Object.keys(d)?.length||d?.length)&&(d[e.name].forEach(i=>t.classList.toggle(i)),p.value=t.className),e.focus()})}),u.forEach(e=>{e.addEventListener("click",()=>{let i,s=e.getAttribute("data-relative");s==="parent"&&(i=t.parentElement?t.parentElement:t,t.parentElement?v(t.parentElement):w("No parent element")),s==="prev"&&(i=t.previousElementSibling?t.previousElementSibling:t,t.previousElementSibling?v(t.previousElementSibling):w("No previous sibling element")),s==="next"&&(i=t.nextElementSibling?t.nextElementSibling:t,t.nextElementSibling?v(t.nextElementSibling):w("No next sibling element")),s==="child"&&(i=t.firstElementChild?t.firstElementChild:t,t.firstElementChild?v(t.firstElementChild):w("No child element")),t=i,c.forEach(o=>o.checked=!0),p.value=t.className,d=h(t)})}),p.addEventListener("keydown",e=>{e.key==="Enter"&&(e.preventDefault(),b(e))}),C.addEventListener("submit",e=>{b(e)}),window.addEventListener("resize",()=>{l.innerText=k()}),$.forEach(e=>{e.addEventListener("click",()=>{a.forEach(i=>r.classList.remove(i)),a=e.getAttribute("data-position").split(","),a.forEach(i=>r.classList.add(i))})});let w=e=>{E.removeAttribute("hidden"),E.innerText=e,setTimeout(()=>E.setAttribute("hidden",!0),3e3)}}},100)}var O=B;export{O as default};
